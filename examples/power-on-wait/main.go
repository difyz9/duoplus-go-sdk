...
func main() {
	...
	if !execute {
		fmt.Println("dry-run only: set DUOPLUS_EXECUTE=1 to actually power on and wait")
		return
	}

	if phone.Status == statusPoweredOn {
		fmt.Println("phone is already powered on")
		return
	}
	...
	fmt.Printf("image_id=%s reached status=%d\n", phone.ID, statusPoweredOn)
}
